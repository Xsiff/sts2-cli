...
def best_card(hand, enemies, osty, energy, rnd, inc, blk, hp):
    ...
    for i, c in enumerate(hand):
        if not c.get("can_play") or c["cost"] > energy:
            continue
        name, cost, ctype, tt = c["name"], c["cost"], c["type"], c.get("target_type", "")
        kw = c.get("keywords") or []
        if ctype in ("Status", "Curse") or "Unplayable" in kw:
            continue
        if name in ["Slimed", "Burn", "Wound", "Dazed", "Infection"]:
            continue
        is_block = ctype == "Skill" and tt != "AnyEnemy"
        is_attack = tt == "AnyEnemy"
        # Priority scoring (lower = play first)
        if cost == 0 and is_block and need_block:
            p = 1  # Free block when needed
        elif cost == 0 and ctype == "Power":
            p = 2  # Free powers always good
        elif cost == 0 and is_attack:
            p = 3  # Free attacks
        elif cost == 0:
            p = 4  # Other free cards
        elif ctype == "Power" and not lethal and inc <= 15:
            p = 8 + cost  # Powers when safe
        elif ctype == "Power" and not lethal:
            p = 15 + cost  # Powers when moderate danger
        elif ctype == "Power" and lethal:
            p = 50  # Don't play powers when dying
        elif is_block and lethal:
            p = 2 + cost  # Block everything when dying
        elif is_block and need_block:
            p = 10 + cost  # Block when needed
        elif is_block and inc == 0:
            p = 45  # Don't block when no damage incoming
        elif is_block and block_gap <= 0:
            p = 42  # Already blocked enough
        elif is_block:
            p = 25 + cost  # Normal block
        elif is_attack and not lethal:
            p = 12 + cost  # Attacks when not dying
        elif is_attack and lethal:
            p = 40  # Don't attack when dying (block instead)
        else:
            p = 20 + cost
        ti = None
...
def fight(d=None):
    ...
    for _ in range(200):
        ...
        if dec == "card_select":
            ...
            d = action(
                "select_cards",
                indices=",".join(str(i) for i in range(min(n, len(d.get("cards", []))))),
            )
            continue
        if dec != "combat_play":
            d = action("proceed")
...
if __name__ == "__main__":
    ...
    if dec == "card_reward":
        ...
        for c in result.get("cards", []):
            print(
                f"  C{c['index']}: {c['name']}({c.get('cost', '?')}) "
                f"{c.get('rarity', '')} {c['type']} {c.get('stats', {})}"
            )
    elif dec == "game_over":
